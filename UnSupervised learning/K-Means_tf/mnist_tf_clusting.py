...
#   train_dir = tempfile.mkdtemp()
  data_sets = read_data_sets('MNIST_data', FLAGS.fake_data)
  ...
  with tf.Graph().as_default():
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      if step % 500 == 0:
        ...
        Clusting_Loss.append(clustering_loss_value)
        STep.append(step)
      ...
      if (step + 1) % 2000 == 0 or (step + 1) == FLAGS.max_steps:
        ...
        plt.plot(STep,Clusting_Loss)
        plt.title('Clusting_Loss')
        plt.xlabel('Step')
        plt.ylabel('Clusting_Loss')
        plt.show()
        # plt.plot(STEP, TRain_prec, 'b-',label = 'Train_Data')
        # plt.plot(STEP, VAL_prec, 'r--', label = 'Val_Data')
        # plt.plot(STEP, TEST_prec, 'g-.', label = 'Test_Data')
